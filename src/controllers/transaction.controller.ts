...
export const sendCoins = async (req: Request, res: Response) => {
    ...
    if (!toUser)
        throw new BadRequestError({ message: "Invalid wallet address" });

    const fee = await getTrxFee(COIN_TRANSACTION_TYPE.TRANSFER, amount);

    if (amount + fee > userBalance)
        throw new BadRequestError({ message: "Insufficient balance" });
...
export const getTransactionHistory = async (req: Request, res: Response) => {
    ...
    const result = await paginate(Transaction, page, limit, [
        { "user.id": req.user.id },
    ]);
